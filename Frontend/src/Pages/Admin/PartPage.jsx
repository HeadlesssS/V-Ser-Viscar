...
export default function PartPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setVendors(
        Array.isArray(vendorsData) ? vendorsData.filter((v) => v.isActive) : [],
      );
    } catch {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      addToast(
        editing ? "Part updated successfully." : "Part added successfully.",
      );
      setShowForm(false);
      load();
    } catch (err) {
  ...
  const handleDelete = async () => {
    try {
      const res = await fetch(`${API}/parts/${confirmDelete.id}`, {
        method: "DELETE",
      });
      if (!res.ok) {
        const err = await res.json();
    ...
    <div className="page">
      ...
      <div className="ph">
        <div>
          ...
          <p className="ph-sub">
            Manage all vehicle parts stocked by the service center.
          </p>
        </div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <ExportPdfButton path="/parts-catalog" filename="parts-catalog.pdf" label="Export PDF" />
      ...
      <div className="stats">
        <div className="sc">
          <span className="sc-n">{parts.length}</span>
          <span className="sc-l">Total Parts</span>
        </div>
        <div className="sc">
          <span className="sc-n sc-n-g">{activeCount}</span>
          <span className="sc-l">Active</span>
        </div>
        <div className="sc">
          <span className="sc-n sc-n-m">{parts.length - activeCount}</span>
          <span className="sc-l">Inactive</span>
        </div>
        <div className="sc">
          <span className="sc-n" style={{ color: "#b05a00" }}>
            {lowStockCount}
          </span>
          <span className="sc-l">Low Stock</span>
        </div>
      </div>
          ...
          {filtered.map((p, i) => {
            ...
            return (
              <div key={p.id} className="card" style={{ position: "relative" }}>
                ...
                <div className="card-rows">
                  {[
                    ...
                    [
                      "📦",
                      "Stock",
                      `${p.stockQuantity} units (min: ${p.lowStockThreshold})`,
                    ],
                ...
                >
                  <span style={{ fontSize: 11, color: "#9c97a3" }}>
                    Profit Margin
                  </span>
                  <span
                    style={{ fontSize: 13, fontWeight: 700, color: "#1a7a3a" }}
                  >
                    {p.costPrice > 0
                ...
                <div className="card-actions">
                  <button className="bic bic-e" onClick={() => openEdit(p)}>
                    ✎ Edit
                  </button>
                  <button
                    className="bic bic-d"
