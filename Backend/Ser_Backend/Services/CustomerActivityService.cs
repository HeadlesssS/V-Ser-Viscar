...
public class CustomerActivityService
{
    ...
    public CustomerActivityService(AppDbContext db, AuditService audit)
    {
        _db = db;
        _audit = audit;
    }
    ...
    public async Task<string> BookAppointmentAsync(int userId, CreateAppointmentDto dto)
    {
        ...
        await _audit.LogAsync("Create", "Appointment",
            $"Appointment booked: {appointment.ServiceType} on {appointment.AppointmentDate:dd MMM yyyy}",
    ...
    public async Task<string> CreatePartRequestAsync(int userId, CreatePartRequestDto dto)
    {
        ...
        await _audit.LogAsync("Create", "PartRequest",
            $"Part request submitted: {request.PartName} (x{request.QuantityRequested})",
            userId, request.Id);
    ...
    public async Task<string> SubmitReviewAsync(int userId, CreateReviewDto dto)
    {
        ...
        await _audit.LogAsync("Create", "Review",
            $"Review submitted ({dto.Rating}/5 stars)",
            userId, review.Id);
    ...
    public async Task<string> DeleteReviewAsync(int reviewId)
    {
        ...
        await _audit.LogAsync("Delete", "Review",
            $"Review #{reviewId} deleted (rating {review.Rating}/5)",
            entityId: reviewId);
    ...
    public async Task<string> UpdateAppointmentStatusAsync(int appointmentId, string status)
    {
        ...
        await _audit.LogAsync("Update", "Appointment",
            $"Appointment #{appointmentId} status changed from {oldStatus} to {status}",
    ...
    public async Task<string> UpdatePartRequestStatusAsync(int requestId, string status)
    {
        ...
        await _audit.LogAsync("Update", "PartRequest",
            $"Part request #{requestId} for '{request.PartName}' changed from {oldStatus} to {status}",
            entityId: requestId,
            oldValues: oldStatus,
            newValues: status);
